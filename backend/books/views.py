...
class BookViewSet(
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            if num_pages <= 0:
                ...
                return Response(
                    {"error": "No pages found in the uploaded PDF file"},
    ...
    def get_books_by_profile(self, request, profile_id, *args, **kwargs):
        try:
            ...
            return Response(
                {"error": "Profile not found for the given profile ID"},
            ...
            logger.error(
                f"Error while retrieving books for profile ID {profile_id}: {str(e)}",
                exc_info=True,
            )
            return Response(
                {"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
    ...
    def bulk_delete(self, request):
        ...
        try:
            ...
            if not book_ids:
                ...
                return Response(
                    {"error": "No book IDs provided for deletion"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
