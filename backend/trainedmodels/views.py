...
class FineTuneViewSet(mixins.CreateModelMixin, viewsets.GenericViewSet):
    ...
    def create(self, request):
        ...
        try:
            ...
            return Response(
                {"message": "Fine-tuning started"}, status=status.HTTP_201_CREATED
            )
            ...
            return Response(
                {"error": "An error occurred while starting fine-tuning"},
    ...
    def get_task_status(self, request):
        ...
        """
        try:
            user_trained_model = UserTrainedModel.objects.get(
            ...
            return Response(
                {"error": "An error occurred while checking task status"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class PredictionViewSet(viewsets.ViewSet):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                trained_model_version = (
                    profile.usertrainedmodel_set.order_by("-created_at").first().version
                )
    ...
    def retrieve(self, request, pk=None):
        ...
        try:
            ...
            word_prediction_mappings = WordPredictionMapping.objects.filter(
    ...
    def get_last_prediction(self, request):
        ...
        try:
            profile = get_object_or_404(UserProfile, user=request.user)
            last_prediction = (
                BookPrediction.objects.filter(book__profile=profile)
                .order_by("-created_at")
                .first()
            )
    ...
    def _extract_unknown_words(self, pdf_path, from_page, to_page, user):
        ...
        try:
            ...
            inputs = tokenizer(
                list(tokens), return_tensors="pt", padding=True, truncation=True
            )
            ...
            unknown_words = [
                word
                for word, predicted_class in zip(tokens, predicted_classes)
    ...
    def _save_prediction_metadata(
        self, profile, trained_model_version, unknown_words, book, from_page, to_page
    ):
        """
