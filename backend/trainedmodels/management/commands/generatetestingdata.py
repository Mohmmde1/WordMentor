...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        input_file_path = os.path.join(
            settings.BASE_DIR, "data", "assessment_words.csv"
        )
        ...
        with open(results_file_path, mode="w", newline="") as results_file:
            ...
            for i in tqdm(range(num_files), desc="Generating files", unit="file"):
                ...
                strategy = known_selection_strategies[
                    i % len(known_selection_strategies)
                ]

                if "known_count" in strategy:
                    # Select the first 'known_count' rows as known
                    df_copy["known"] = [
                        "known" if idx < strategy["known_count"] else "not known"
                    ...
                    df_copy["known"] = [
                        "known" if idx in indices else "not known"
                        for idx in df_copy.index
                    ]
                ...
                average_difficulty_known = df_copy[df_copy["known"] == "known"][
                    "difficulty_level"
                ].mean()
                average_difficulty_not_known = df_copy[df_copy["known"] == "not known"][
                    "difficulty_level"
                ].mean()
                ...
                for level in range(1, 11):  # Difficulty levels from 1 to 10
                    count_known_level = df_copy[
                        (df_copy["known"] == "known")
                        & (df_copy["difficulty_level"] == level)
                    ].shape[0]
        ...
        self.stdout.write(
            self.style.SUCCESS(f"{num_files} files generated successfully!")
        )
